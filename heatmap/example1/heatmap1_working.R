...
# Read in data files
test = as.matrix(read.table(file="test.txt",header=TRUE,sep="\t",check.names=FALSE, strip.white=TRUE))
annotation_row = as.data.frame(read.table(file="annotation_row.txt",header=TRUE,sep="\t",check.names=FALSE, strip.white=TRUE))
annotation_col = as.data.frame(read.table(file="annotation_col.txt",header=TRUE,sep="\t",check.names=FALSE, strip.white=TRUE))

# define orders of annotation
annotation_col$Dose <- factor( annotation_col$Dose, levels = c("10 MG", "30 MG", "100 MG", "200 MG", "400 MG", "800 MG"))
annotation_col$Visit_Type <- factor( annotation_col$Visit_Type, levels = c("H4","H8", "H12"))
